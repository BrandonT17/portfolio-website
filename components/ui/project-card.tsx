...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <div className="flex flex-col md:flex-row justify-between gap-5 md:text-start text-center boxx bg-white">
      <div className="flex flex-col md:w-3/5 p-3 justify-center gap-1 md:order-1 order-2">
        ...
        <p className="flex flex-wrap gap-2 text-sm md:justify-start justify-center pt-2">
          {topics.map((topic) => (
            <span
              key={topic}
              className="border-4 border-double px-1 hover:bg-black hover:text-white text-sm"
            >
              {topic}
            </span>
          ))}
